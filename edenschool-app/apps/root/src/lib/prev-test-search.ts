...
/**
 * 내신 기출 검색 — 대시보드(/admin/prev-test-dashboard)와
 * 관리 페이지(/admin/prev-test-add) 하단 목록에서 공용으로 사용한다.
 */

export interface PrevTestRow {
  id: number;
  school_name: string;
  grade: number;
  year: number;
...
export interface PrevTestSearchParams {
  year?: string;
  search?: string;
  school?: string;
  section?: string;
...
export interface PrevTestSearchResult {
  region: string;
...
export async function searchPrevTests(params: PrevTestSearchParams): Promise<PrevTestSearchResult> {
  ...
  return { region, testList, schools, publishers, years };
}
