/**
 * 「이든 진로탐색 도우미」 메뉴의 학원 측 콘텐츠와 코드표.
 *
 * API 가 주지 않는 것(국어 과목 설명 카드, 수행평가 구성 틀 등)을 여기 모은다.
 * 클라이언트 컴포넌트에서도 import 하므로 서버 전용 모듈(db, fetch 클라이언트)을 끌어오지 않는다.
 *
 * ※ 과목 설명·수행평가 틀은 기획서 기준으로 작성한 초안이다. 학원 측 지도·서비스 내용은 검증 전이므로 넣지 않는다.
 */
 ...
 * ※ 학기마다 보완하는 목록이다. 비어 있는 계열은 화면에 "준비 중"으로 표시된다.
 */
