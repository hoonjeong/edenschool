...
export const POST = withErrorHandler(async (req: NextRequest) => {
  ...
  const normalizedSphone = normalizePhone(sphone || '');
  const normalizedPphone = normalizePhone(pphone || '');
  if ((normalizedSphone && !isValidPhone(normalizedSphone)) || (normalizedPphone && !isValidPhone(normalizedPphone))) {
