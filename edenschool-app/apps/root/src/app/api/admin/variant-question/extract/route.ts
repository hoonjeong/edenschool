...
export const POST = withErrorHandler(async (req: NextRequest) => {
  ...
  const { ids, source } = body as { ids: number[]; source: 'prev-test' | 'split-file' };
  ...
  if (source !== 'prev-test' && source !== 'split-file') {
    return NextResponse.json({ error: 'source는 prev-test 또는 split-file이어야 합니다.' }, { status: 400 });
  }

  // DB에서 파일 조회
  const files =
    source === 'prev-test'
      ? await selectPrevTestFileInfoByInfoIds(ids)
      : await selectSplitFileContentsByMetaIds(ids);
