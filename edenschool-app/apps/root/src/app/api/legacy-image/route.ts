...
// 레거시(구 JSP) 호환: 옛 게시물 본문이 참조하는 `image-view.html?id=<file_info.id>`를
// 처리한다. next.config의 rewrite가 /image-view.html → 이 라우트로 보낸다.
// 게시판은 공개 페이지이므로 인증 없이 서빙하되, 노출 범위를 줄이기 위해
// 이미지 확장자만 응답하고(첨부 PDF 등은 404) 다른 file_info 유출을 막는다.
