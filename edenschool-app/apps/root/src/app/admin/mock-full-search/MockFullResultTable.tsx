...
export default function MockFullResultTable({ list }: { list: Row[] }) {
  ...
  async function bulkDownload() {
    ...
    setDownloading(true);
    try {
      const res = await fetch('/api/admin/mock-full/download-bulk', {
      ...
      a.href = url; a.download = '풀세트모의고사_일괄다운.zip';
        ...
        <table className="table table-bordered table-hover table-sm">
          <thead className="thead-dark">
            <tr>
              ...
              <th style={{ width: 70 }}>년도</th>
              <th style={{ width: 70 }}>시행월</th>
              <th>파일명</th>
            </tr>
          </thead>
          <tbody>
            {list.length === 0 ? (
              <tr><td colSpan={5} className="text-center text-muted py-3">모의고사가 없습니다.</td></tr>
            ) : (
              list.map((item, idx) => (
                <tr key={`${item.id}-${idx}`}>
                  ...
                  <td>
                    {item.contentId ? (
                      <a href={`/api/admin/mock-full/download?id=${item.id}`}>{item.fileName}</a>
                    ) : (
