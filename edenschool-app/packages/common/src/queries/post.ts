...
export async function selectPostInfoList(code: string, category?: string): Promise<PostInfo[]> {
  let sql = `SELECT id, subject, contents, code, category, user_id as userId, read_count as readCount, date_format(insert_time, "%Y.%m.%d") as date, (SELECT count(0) as cnt FROM comment WHERE post_id=p.id) as commentCount FROM post_info p WHERE code=?`;
